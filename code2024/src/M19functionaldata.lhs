...
Write $r(t)$ for the rank of node $t$.

Let $n > 0$ the size of a leftist heap rooted in node $t$ with left and right subtrees
rooted in $t.l$ and $t.r$, respectively. In this case,
we have $l + r + 1 = n$ for some natural numbers $l= size(t.l)$ and $r= size(t.r)$.
In a leftist heap, we also have the rank condition $r(t.r)\le r(t.l)$.
Now we calculate the rank of $t$:

$r(t) = r(t.r) + 1 \le \floor(\log_2 (r+1)) + 1 = \floor(\log_2 (2r+2)) \le \floor(\log_2 (n+1))$
...
size n correspond to the one-bits in the binary representation of n.
